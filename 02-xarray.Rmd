...
## Creating a function with Xarray

Not let's create a simple function on xarray. First, we should import the key libraries for creating a function that calculates the monthly mean temperature from daily data for each point in space defined by its coordinates ```latitude``` and ```longitude```.

This example shows how to use Numpy functions, such as ```mean()``` an ```std()```, in a xarray Dataset.
...
![Figure 1 - Monthly mean temperature](output_11_0.png)
...
![Figure 2 - Seasonal mean temperature](output_17_0.png)
    
## Calculating the multi-model ensemble mean
...
![Figure 3 - Mean temperature calculated for each GCM](output_15_0.png)
...
![Figure 4 - Multi-model ensemble mean temperature](output_19_1.png)

## Calculating autocorrelation lag-1

In the last two sections, we saw how to use well-defined NumPy operations ```mean()``` and ```std()``` in a Dataset over the dimension ```time```. Now, how would we create a new function that is not defined in the NumPy library? Let's try first with the lag-1 autocorrelation. 

First, let'screate a new function ```lag1_numpy``` by using the NumPy syntax.
...
![Figure 5 - Multi-model ensemble lag-1 autocorrelation](output_16_1.png)
